...
async def init_db():
    async with engine.begin() as conn:
        ...
        await conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS name_en VARCHAR(255)"))
        await conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS avatar_url VARCHAR(500000)"))
        await conn.execute(text("ALTER TABLE users ALTER COLUMN avatar_url TYPE VARCHAR(500000)"))
        await conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS phone VARCHAR(64)"))
        await conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS phone_ext VARCHAR(32)"))
        await conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS location VARCHAR(255)"))
        await conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS time_zone VARCHAR(255)"))
        await conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS employee_id VARCHAR(64)"))
        ...
        await conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS team VARCHAR(255)"))
        await conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS reports_to VARCHAR(255)"))
        await conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS joined_at VARCHAR(64)"))
        await conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS bio VARCHAR(2048)"))
        await conn.execute(text("ALTER TABLE users ADD COLUMN IF NOT EXISTS languages VARCHAR(512)"))
